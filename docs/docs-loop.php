		...
		<?php if ( ! empty( $_GET['folder'] ) ) : ?>
			<tr class="folder-row">
				...
				<td class="folder-row-name" colspan=10>
					<a href="<?php echo esc_url( bp_docs_get_parent_folder_url() ) ?>" class="up-one-folder"><span class="screen-reader-text"><?php _e( 'Go up one folder', 'buddypress-docs' ) ?></span><?php bp_docs_genericon( 'category', 0 ); ?><?php _ex( '.. ', 'up one folder', 'buddypress-docs' ) ?></a>
				</td>
			</tr>
		<?php endif ?>
		...
		<?php if ( bp_docs_include_folders_in_loop_view() ) : ?>
			<?php foreach ( bp_docs_get_folders() as $folder ) : ?>
				<tr class="folder-row">
					...
					<?php if ( bp_docs_enable_attachments() ) : ?>
						<td class="attachment-clip-cell">
							<?php bp_docs_attachment_icon() ?><?php // bp_docs_genericon( 'category', $folder->ID ); ?>
						</td>
					<?php endif ?>
