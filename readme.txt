=== Social Learner ===
Contributors: BuddyBoss, Joachim Happel, rpi-virtuell
Requires at least: 3.8
Tested up to: 4.9.7
Version: 1.3.2
License: GPLv3
License URI: http://www.gnu.org/licenses/gpl-3.0.html

== Description ==

A child theme for the Boss theme, designed to be used in conjunction with Boss for LearnDash plugins.

== Required Plugins:  ==

* BuddyPress
* Boss Theme (Buddyboss.com)
* bbpress
* BuddyPress Forum Editor
* BP Group Hierarchy
* Advanced Tiny-MCE
* BuddyPress Docs
* BuddyPress Global Search


== Installation ==

1. Make sure BuddyPress and the Boss theme are activated.
2. Visit 'Appearance > Themes'
3. Click 'Add New'
4. Upload the file 'social-learner.zip'
5. Activate Social Learner from your Themes page.

== Changelog ==
= 1.3.2 =
* add html 5 download attribute to docs attacmend single

= 1.3.1 =
*   buddypress-docs fix language issues 

= 1.3.0 =
*  new language files in buddypress-docs fix language issues

= 1.2.9 =
*  order docs attachments by title #193

= 1.2.8 =
* skip group creation after step 2 and configure it later

= 1.2.7 =
* remove "leave group" but not "join group" button on group header

= 1.2.6 =
* Fix: BuddyPress sidebar lists multiple blogs.

= 1.2.5 =
* replaces BP Group Hierarchy with https://github.com/dcavins/hierarchical-groups-for-bp from David Cavins
* force compatibillity with bp 2.8

= 1.2.4 =
* deny "Tiny MCE Advanced" in forums

= 1.2.3 =
* CSS Simple Calendar Integration
* Fixed fancy fox issue (conflict with global search box)

= 1.2.2 =
* CSS Fixes

= 1.2.1 =
* workaround to show styled radio buttons in chrome

= 1.2.0 =
* fix for use with buddypress 2.7

= 1.1.9 =
* add "kitchen sink" button to tiny-mce Toolbar for docs edit

= 1.1.8 =
* remove "leave group" button on group header. https://github.com/rpi-virtuell/reliwerk/issues/117

= 1.1.7 =
* fix firefox preload behavior with 403 on bp-doc save
* add bp-doc fullscreenmode 
* fixed tiny-mce buttons for bp-doc  editor
* fix wrong attachment display for png an gif unter bp-docs

= 1.1.6 =
* css fix for chrome in groups activity filter

= 1.1.5 =
* removed min-width for smartphone/mobile view.

= 1.1.4 =
* changed activity filter to jquery checkbox select in member activity
* save filter settings in user metadata

= 1.1.3 =
* changed activity filter to jquery checkbox select in activity and group activity views

= 1.1.2 =
* fix: could not delete attached images https://github.com/rpi-virtuell/reliwerk/issues/120

= 1.1.1 =
* several css fixes
* fix delete button for attached images in buddypress docs

= 1.1.0 =
* add buddypress docs folder support
* fix group cover photo

= 1.0.9 =
* add buddypress docs tree support
* add groups menu collaps
* change single docs layout
* display docs attachment images
* reduce docs nav items

= 1.0.8 =
* fix check honeypot

= 1.0.7 =
* removed shortcodes to Group Description
* add Widget or Sidebar Shortcode for sticky activity plugin to template

= 1.0.6.1 =
* added shortcodes to Group Description
* css fix for "Widget or Sidebar Shortcode" Plugin in Group Description

= 1.0.6 =
* Removed Boxed layout option
* Fixed Logo background color issue
* Fixes for Sensei
* Color fixes

= 1.0.5 =
* Fixed Dashboard links not visible in Profile Menu

= 1.0.4 =
* Compatibility with Boss 2.0.0
* New - Boxed layout option
* New - Support for RTL (Right to Left) languages
* Added modular header code for easier theme updates
* Various CSS fixes
* Added translation .POT file
* Standard readme.txt format

= 1.0.3 =
* Added language files
* Added French translations, credits to Jean-Pierre Michaud
* Various CSS fixes
* Fixed dropdown color
* HTML tag is more Validator friendly
* WooCommerce buttons migrated into Boss theme
* WooCommerce cart icon added for logged-out users
* Fixed layout with sidebar for mobile

= 1.0.2 =
* Compatibility with LearnDash
* layout fixes

= 1.0.1 =
* Fixed error with BadgeOS not installed

= 1.0.0 =
* Initial public release
